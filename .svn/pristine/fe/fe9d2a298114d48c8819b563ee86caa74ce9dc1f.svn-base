...
<form:form id="fenye" name="fenye" action="${ctx}/admin/product/query" method="post">
    <div id="content_top">
        <table class="jymf_table" >
            <tr class="table_1_tr">
                ...
                <td><input class="center_btn " style="margin-left: 30px;" type="submit" value="查 询"/></td>
                <td><input class="center_btn " style="margin-left: 30px;" id ="addBtn" type="button" value="新 增"/></td>
                <td><input class="center_btn " style="margin-left: 30px;" type="button" value="产品类型" 
                			onclick="location.href='${ctx}/admin/product/productType/${company.id}/0/${company.workMode}'"/>
                </td>
                <td><input class="center_btn " style="margin-left: 30px;" id ="backBtn" type="button" value="返 回"/></td>
            </tr>
        </table>
    </div> 
    ...
    <table id="table_1" class="jymf_table table_border">
        <tr id="table_1_titlebar">
            ...
            <td class="jymf_td row_1" rowspan="1" colspan="1">克隆</td>
        </tr>
        ...
        <tr class="jymf_tr table_1_tr">
            ...
            <td  align="center">
                <div class="small_btn">
                    <a class="linke" href="${ctx}/admin/product/update/${product.id}" 
                       id="editLink-${product.id}">
                       <c:if test="${product.status ne '1'}">审核</c:if>
                       <c:if test="${product.status eq '1'}">修改</c:if>
                    </a>
                </div>
            </td>
            <td  align="center">
                <div class="small_btn">
                    <a class="linke" href="${ctx}/admin/product/clone/${product.id}" 
                       id="editLink-${product.id}">克隆</a>
                </div>
            </td>
        </tr>
        </c:forEach>
