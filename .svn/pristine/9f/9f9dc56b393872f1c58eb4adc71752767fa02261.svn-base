...
<div class="row">
    ...
    <div class="box-content">
   		<div  class="alert alert-info" style="padding:5px;margin-bottom:5px;">
			<form:form id="fenye" class="form-inline" action="${ctx}/monitor/count/queryArrivalPort" method="post">
				<div class="form-group">
			      	<label>到达口岸&nbsp;&nbsp;</label>
					 <select class="chzn-select form-inline input-sm" name="arrivalPort">
					 	<option value="">全部选择</option>
			        	<c:forEach items="${ports}" var="list">
			        		<option value="${list.portName}"<c:if test="${list.portName==count.arrivalPort}">selected="selected"</c:if> >${list.portName}</option>
			        	</c:forEach>
			        </select>&nbsp;&nbsp;
			  	</div>    
			   	<div class="form-group">
