...
$(document).ready(function(){
	...
	$("#cancel_save").click(function(){hidePicdiv()});
...
function certain_save(){
	...
	parent.document.getElementById("win").contentWindow.$("#imgFile"+pic_order+"Name").val($("#pic_name").val());
	parent.document.getElementById("win").contentWindow.$("#img"+pic_order).attr("src",pic_url);
	parent.$("#bg").hide();
	parent.$("#showPic").hide();
}
...
<body >
	<div style="padding:0px; margin:0px;height:30px;">	
		...
		<a href="javascript:hidePicdiv();" style="float:right"><img style="border:0px;" src="${ctx}/static/images/x.png" /></a>
	</div>
	...
	<div align="center" style="padding:15px; background:#EFEFEF;height:450px;">
     ...
     <div align="center" style="margin-top: 30px;">
     	<form:form id="upload_pic"  action="${ctx}/common/upload" method="post" enctype ="multipart/form-data">
			...
			<input type="file" accept="image/jpeg,image/x-png" name="pic_file" id="pic_file"  style="height: 30px;filter:alpha(opacity:0);opacity: 0;width:82px;position:absolute ;left: 113px;bottom: 50px;" onchange="changePicUrl(this)">
			<input id="ask_pic" type="hidden" value="${ctx}/common/getPic/${pic_url}" /> 
		    <button type="button" class="btn btn-primary" id="crop_submit" >剪&nbsp;&nbsp;&nbsp;&nbsp;裁</button> 
		    <button type="button" class="btn btn-primary" id="certain_save">保&nbsp;&nbsp;&nbsp;&nbsp;存</button> 
		    <button type="button" class="btn btn-default" id="cancel_save">取&nbsp;&nbsp;&nbsp;&nbsp;消</button><br><br>
		</form:form>
