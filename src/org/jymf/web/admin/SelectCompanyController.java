...
public class SelectCompanyController {
	...
	public String selectProduct(Model model,
		...
		company=companyService.findById(new BigDecimal(companyId),false);
		
		pageView = productService.selectProduct(pageView, product, company.getWorkMode());
	...
	@RequestMapping(value = "selectPrelabeledProduct/{companyId}")
	public String selectPrelabeledProduct(Model model,
			@PathVariable("companyId") String companyId, LabelIndex labelIndex, String pageNow) {
		PageView pageView = null;
		if (Common.isEmpty(pageNow)) {
			pageView = new PageView(1);
		} else {
		...
		pageView = labelIndexService.queryUsablePreLabel(pageView, labelIndex, company.getWorkMode());
		
		model.addAttribute("labelIndex", labelIndex);
		model.addAttribute("pageView", pageView);
		
		return "/admin/dialog/selectPrelabeledProduct";
	}
