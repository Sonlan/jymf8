...
public class CompanyUserServiceImpl implements ICompanyUserService{
    ...
    public void add(CompanyUser companyUser) {
    	...
    	try {
			companyUser.setPassword(StringUtils.isNotEmpty(companyUser.getPassword())?MD5.getInstance().encrypt(companyUser.getPassword()):MD5.getInstance().encrypt("123456"));
		} catch (Exception e) {
			return;
		}
	...
	public CompanyUser findUser(CompanyUser companyUser) {
		...
		return companyUserDao.findUserByLicenseAndPwd(companyUser);
	}
